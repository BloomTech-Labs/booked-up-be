        ...
        Users.findAgentInfoId(req.params.id)
          .then((agentInfo) => {
            const infoList = agentInfo.map((info) => {
              const {
                agent_type, agency_type, agency_address, agency_phone_number, agency_email,
              } = info;
              return {
                agent_type, agency_type, agency_address, agency_phone_number, agency_email,
              };
...
router.patch('/:id/email', [
  ...
  body('email').custom((value, { req, loc, path }) => Users.findByEmail(value).then((user) => {
    if (user.length === 0) {
      return null;
...
router.patch('/:id/displayName', [
  ...
  body('display_name').custom((value, { req, loc, path }) => Users.findByDisplayName(value).then((user) => {
    if (user.length === 0) {
      return null;
    } if (user[0].display_name === value && Object.is(Number(req.params.id), user[0].id)) {
